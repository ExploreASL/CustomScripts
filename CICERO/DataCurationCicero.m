...
if ispc
    ...
    Rdir = '/Users/henk/surfdrive/HolidayPics/CICERO_Nolan';
end
...
%         %% Move RPI 4V_ResultFiles
%         RPIdir = fullfile(AnalysisSubjectDir, 'RPI_4V');
%         xASL_adm_CreateDir(RPIdir);
%         
%         % Now set orientation of all files to the new orientation
%         DestPath = fullfile(AnalysisSubjectDir,'T1.nii');
%         
% %         ListIs{1} = fullfile(SubjectDir,'DARTEL','COREG','T1.nii'); % can
% %         be used for registration
%         ListIs{1} = fullfile(SubjectDir,'DARTEL','4V','4V_MAP.nii');
%         ListIs{2} = fullfile(SubjectDir,'DARTEL','TASL','4V.nii');
%         ListIs{3} = fullfile(SubjectDir,'DARTEL','TASL','CoW_MAP.nii');
%         ListIs{4} = fullfile(SubjectDir,'DARTEL','TASL','HEMI.nii');
%         ListIs{5} = fullfile(SubjectDir,'DARTEL','TASL','TASL.nii');
% 
%         if ~xASL_exist(DestPath,'file')
%             warning(['File missing:' DestPath]);
%             continue;
%         end
% 
%         DestNii = xASL_io_ReadNifti(DestPath);        
%         
%         for iList=1:length(ListIs)
%             % Check file existence
%             if ~xASL_exist(ListIs{iList},'file')
%                 warning(['File missing:' ListIs{iList}]);
%                 continue;
%             end
% 
%             % Set orientation
%             niiIs = xASL_io_ReadNifti(ListIs{iList});
%             niiIs.mat = DestNii.mat;
%             create(niiIs);
%             clear niiIs
%             
%             [~, Ffile, Fext] = xASL_fileparts(ListIs{iList});
%             NewPath = fullfile(RPIdir, [Ffile Fext]);
%             xASL_Copy(ListIs{iList}, NewPath, true);
%         end
        
        %% Delete residual folders
        FoldersAre = {'DARTEL' 'RPI' 'NICE4V' 'flow' 'DEEPGM' 'HDYNSTEAL' 'stock' 'FIG'};
        for iFolder=1:length(FoldersAre)
            FolderDir = fullfile(SubjectDir, FoldersAre{iFolder});
            xASL_adm_DeleteFileList(FolderDir, '^.*$', 1);
            xASL_delete(FolderDir);

            xASL_delete(fullfile(SubjectDir,'.DS_Store'));
        end
        if isempty(xASL_adm_GetFileList(SubjectDir,'^.*$','FPListRec',[0 Inf]))
            xASL_delete(SubjectDir);
        else
            warning(['Not empty:' SubjectDir]);
        end
    end % Subjects
end % Cohorts
fprintf('\n');
...
%% Delete Nolan segmentations that we wont use
xASL_adm_DeleteFileList(AnalysisDir, '^Nolan.*c(1|2|3).*\.nii$',1);
xASL_adm_DeleteFileList(AnalysisDir, '^T1_seg8\.mat$',1);
...
% Lower resolution DARTEL didnt help but is fine

%% Warp also the RPI results to standard space, after loading the DataPar file
for iSubject=1:x.nSubjects
    SubjectDir = fullfile(x.D.ROOT,x.SUBJECTS{iSubject});
    DeformationPath = fullfile(SubjectDir,'y_T1.nii');
    FileList = {'4V_MAP' '4V' 'CoW_MAP' 'HEMI' 'TASL'};
    PathIn = cellfun(@(z) fullfile(SubjectDir,'RPI_4V', [z '.nii']), FileList, 'UniformOutput',false);
    PathOut = cellfun(@(z) fullfile(x.D.PopDir,[z '_' x.SUBJECTS{iSubject} '.nii']), FileList, 'UniformOutput',false);
    
    xASL_spm_deformations(x, PathIn, PathOut, 1, [], [], DeformationPath);
end
