...
% ndnanfilter uses a default alpha=2.5, which is good because it keeps your window
% well related to the SD - not too big not too small
% So the best is only changing the N parameter

% VoxelSize   = 1.5;
% FwHm2SD     = (2*(2*reallog(2))^0.5);
% 
% FWHM        = [3.5 6.4 9.2 12 14.8];                           % mm smoothing kernel (3D)
% SD          = FWHM./FwHm2SD;
% N           = ((SD.*5./VoxelSize)-1)./2; % cave seems to work better if N is even
% 
% SD = (2*N+1)/5*VoxelSize
% FWHM = (2*N+1)/5*VoxelSize*FwHm2SD
% 
% pragmatically, ndNaNFilter seems to work better if N is even
% @ VoxelSize 1.5 mm:
% N= 1 == 2.1 mm FWHM
% N= 2 == 3.5 mm FWHM 
% N= 4 == 6.4 mm FWHMN
