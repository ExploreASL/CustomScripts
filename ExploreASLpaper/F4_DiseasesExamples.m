...
% SAME WITH OLD PIPELINE
ROOT{1}     = '/Users/henk/ExploreASL/ASL/ExploreASL_Manuscript/FeaturesDisabled_NOVICE/Population/Templates'; % diseased kids
ROOT{2}     = '/Users/henk/ExploreASL/ASL/ExploreASL_Manuscript/FeaturesDisabled_Sleep/Population/Templates'; % healthy adults
ROOT{3}     = '/Users/henk/ExploreASL/ASL/ExploreASL_Manuscript/FeaturesDisabled_EPAD/Population/Templates'; % diseased elderly
...
Path_pCSF = fullfile(ROOT{2},'pCSF_bs-mean.nii');

if ~xASL_exist(Path_pCSF)
    bMask = (pGM+pWM)>0.1;
    bMask = xASL_im_DilateErodeFull(bMask,'dilate',xASL_im_DilateErodeSphere(1));
else
    pCSF = xASL_io_Nifti2Im(Path_pCSF);
...
for iScanType=1:length(Modalities)
    ...
    for iStudy=1:length(ROOT)
        ...
        if ProcessThis
            IM{iStudy} = xASL_vis_CreateVisualFig(x, tIM, [], Intens{iScanType}, [], ColorScale{iScanType},false,MasksAre{iScanType},bWhite, MaxWindow{iScanType});
