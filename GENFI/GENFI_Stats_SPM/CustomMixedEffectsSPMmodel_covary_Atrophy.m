...
%% Create voxel coordinates to track (within mask)
...
MaskIM                                                  = zeros(121,145,121);
MaskIM(VoxelCoor(iC,1),VoxelCoor(iC,2),VoxelCoor(iC,3)) = 1;
...
MaskIM                                                          = zeros(121,145,121);
MaskIM(MaskIM==0)                                               = NaN;
MaskIM(VoxelCoor(iVox,1),VoxelCoor(iVox,2),VoxelCoor(iVox,3))   = 1;
xASL_io_SaveNifti( fullfile( SPMdir, 'maskNII.nii'), fullfile( SPMdir, 'maskNII.nii'), MaskIM);
...
% %% Create single-voxel Niftis with voxel 50,50,50
%
% Xcoor   = 50;
% Ycoor   = 50;
% Zcoor   = 50;
...
%
% % CreateMask
% FilePathNew     = fullfile( SPMdir, 'maskNII.nii');
% dummyIM         = zeros(3,3,3);
% dummyIM(2,2,2)  = 1;
