...
x.S.CoVar(:,nGeneticStatus)=x.S.CoVar(:,nGeneticStatus)+1;
...
%% Find sets indices
SetsNames   = {'MutationStatus7' 'Family' 'sex' 'Yrs_AAO' 'age' 'Site' 'CBF_spatial_CoV' 'GeneticStatus'};
...
matlabbatch{1}.spm.stats.factorial_design.des.fblock.fac(1).name    = x.S.SetsName{nGeneticStatus}; % MutationStatus7
...
matlabbatch{1}.spm.stats.factorial_design.des.fblock.fsuball.specall.imatrix    = double([ones(length(x.S.CoVar(:,nGeneticStatus)),1) x.S.CoVar(:,nGeneticStatus) ones(length(x.S.CoVar(:,nGeneticStatus)),1)  ones(length(x.S.CoVar(:,nGeneticStatus)),1)]);
...
matlabbatch{1}.spm.stats.factorial_design.cov(2).iCFI           = 1; % 1==no interactions, 2 = with factor 1
...
% matlabbatch{1}.spm.stats.factorial_design.cov(3).c              = double(x.S.CoVar(:,nSite)); % vector data
% matlabbatch{1}.spm.stats.factorial_design.cov(3).cname          = x.S.SetsName{nSite}; % Site
% matlabbatch{1}.spm.stats.factorial_design.cov(3).iCFI           = 1; % 1==no interactions, 2 = with factor 1
% matlabbatch{1}.spm.stats.factorial_design.cov(3).iCC            = 1; % 5==no mean centering
...
%% 3    Contrast creation
    ...
    for ii=1
        matlabbatch{1}.spm.stats.con.consess{ii}.tcon.weights       = zeros(1,5);
        matlabbatch{1}.spm.stats.con.consess{ii}.tcon.sessrep       = 'none';
    end


%     t-test
    matlabbatch{1}.spm.stats.con.consess{1}.tcon.name               = 'T-test';
    matlabbatch{1}.spm.stats.con.consess{1}.tcon.weights(1)         = -1;
    matlabbatch{1}.spm.stats.con.consess{1}.tcon.weights(2)         =  1;
...
%     matlabbatch{1}.spm.stats.con.consess{4}.tcon.weights(12)        =  1;
    ...
    matlabbatch{1}.spm.stats.results.conspec.extent     = 0;  % width threshold (k)
