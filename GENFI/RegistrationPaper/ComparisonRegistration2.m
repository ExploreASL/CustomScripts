...
Vendors = {'GE' 'PHBsup'}; % first two vendors, to save memory
%  'PHnonBsup' 'SI'
...
GEcoeffGM           = nonzeros(squeeze(TCoeffMedian(:,1,:,:,5)))
PHcoeffGM           = nonzeros(squeeze(TCoeffMedian(:,2,:,:,5)))
GEcoeffGM           = nonzeros(squeeze(TCoeffMAD(:,1,:,:,5)))
PHcoeffGM           = nonzeros(squeeze(TCoeffMAD(:,2,:,:,5)))
...
for iR=1:length(TOTALIM)
    for iV=1:length(TOTALIM{iR})
        for iMask=1:length(GMMASK)
            if ~isempty(strfind(RegCat{iR },'PWI_pGM'))
                figure(1);plot(mean(N{iR}{1}{iV}(:,:,iMask),2),mean(X{iR}{1}{iV}(:,:,iMask),2),'r',mean(N{iR}{2}{iV}(:,:,iMask),2),mean(X{iR}{2}{iV}(:,:,iMask),2),'k',mean(N{iR}{3}{iV}(:,:,iMask),2),mean(X{iR}{3}{iV}(:,:,iMask),2),'g');
            else
